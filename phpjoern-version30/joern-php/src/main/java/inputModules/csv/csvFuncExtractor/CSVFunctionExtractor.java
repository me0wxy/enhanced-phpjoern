...
import filesystem.FSEdge;
import filesystem.FSNode;
...
public class CSVFunctionExtractor
{
	...
	private void addNodeRowsUntilNextFile() throws InvalidCSVFile
	{
		while( nodeReader.hasNextRow())
		{
			...
			if( currType.equals(PHPCSVNodeTypes.TYPE_DIRECTORY)){
				addNodetoFileSystem(currNodeRow);
				continue;
			}
	...
	private void addEdgeRowsUntilNextFile() throws InvalidCSVFile
	{
		...
		while( edgeReader.hasNextRow())
		{
			...
			if( currType.equals(PHPCSVEdgeTypes.TYPE_DIRECTORY_OF)){
				addEdgetoFileSystem(currEdgeRow);
				continue;
			}
	...
	//mxy
	private void addNodetoFileSystem(KeyedCSVRow row){
		...
		newNode.setId(id);
		PHPIncludeMapFactory.addNewFileNode(newNode);
	}
	private void addEdgetoFileSystem(KeyedCSVRow row){
		...
		PHPIncludeMapFactory.addNewDirectoryEdge(startid, endid);
	}
}
