...
public class PHPCSVEdgeInterpreter implements CSVRowInterpreter
{
		...
		throws InvalidCSVFile
	{
		...
		switch (type)
		{
			...
			case PHPCSVNodeTypes.TYPE_CLASS_NAME:
				errno = handleClassName((ClassNameExpression)startNode, endNode, childnum);
				break;
			...
			case PHPCSVNodeTypes.TYPE_USE:
				...
				// statement version 70
			case PHPCSVNodeTypes.TYPE_PROP_GROUP:
				errno = handlePropertyGroup((PropertyGroup)startNode, endNode, childnum);
				break;

			case PHPCSVNodeTypes.TYPE_NULLABLE_TYPE:
				errno = handleNullableType((NullableType)startNode, endNode, childnum);
				break;

			case PHPCSVNodeTypes.TYPE_ARROW_FUNC:
				errno = handleArrowFunction((ArrowFuncExpression)startNode, endNode, childnum);
				break;

				// ast.so 80 New Features
			case PHPCSVNodeTypes.TYPE_TYPE_UNION:
				errno = handleUnionType((UnionType)startNode, endNode, childnum);
				break;

			case PHPCSVNodeTypes.TYPE_ATTRIBUTE_LIST:
				errno = handleAttributeList((AttributeList)startNode, endNode, childnum);
				break;

			case PHPCSVNodeTypes.TYPE_ATTRIBUTE_GROUP:
				errno = handleAttributeGroup((AttributeGroup)startNode, endNode, childnum);
				break;

			case PHPCSVNodeTypes.TYPE_ATTRIBUTE:
				errno = handleAttribute((Attribute)startNode, endNode, childnum);
				break;

			case PHPCSVNodeTypes.TYPE_MATCH:
				errno = handleMatch((MatchExpression)startNode, endNode, childnum);
				break;

			case PHPCSVNodeTypes.TYPE_MATCH_ARM_LIST:
				errno = handleMatchARMList((MatchARMList)startNode, endNode, childnum);
				break;

			case PHPCSVNodeTypes.TYPE_MATCH_ARM:
				errno = handleMatchARMExpression((MatchARMExpression)startNode, endNode, childnum);
				break;

			case PHPCSVNodeTypes.TYPE_NULLSAFE_PROP:
				errno = handleNullsafeProperty((NullsafePropertyExpression)startNode, endNode, childnum);
				break;
	...
	private int handleFunction( FunctionDef startNode, ASTNode endNode, int childnum)
	{
		...
		switch (childnum)
		{
			case 0:	// name chile: String Expression node
				startNode.setFunctionName((StringExpression)endNode);
				break;
			case 1:	// docComment child: StringExpression node or NULL node
				if (endNode instanceof NullNode)
					startNode.addChild(endNode);
				else
			...
			case 4: // returnType child: either Identifier or NULL node
				...
				break;
			case 5: // attribute child: either AttributeList or NullNode
				if ( endNode instanceof NullNode)
					startNode.addChild(endNode);
				else
					startNode.setAttributeList((AttributeList)endNode);
				break;
			case 6: // integer chAttributeGroupild: IntegerExpression node
		...
		switch (childnum)
		{
			...
			case 4: // returnType child: either Identifier or NULL node
				if( endNode instanceof NullNode)
					...
					startNode.setReturnType((Identifier)endNode);
				break;
			case 5:	// attribute child: either Attribute or NullNode
				if ( endNode instanceof NullNode)
					startNode.addChild(endNode);
				else
					startNode.setAttributeList((AttributeList)endNode);
				break;
			case 6: // IntegerExpression child
				startNode.setOffset((IntegerExpression)endNode);
				break;
...
//				startNode.setNamewithNS(startNode.getNameSpace(), classname);
			...
			case 1: // docComment child: either StringExpression or Null node
				if ( endNode instanceof NullNode)
					...
					startNode.setClassDocComment((StringExpression) endNode);
				 break;
			...
			case 5:	// attribute child: either AttributeList or NullNode
				if ( endNode instanceof NullNode)
					...
					startNode.setAttributeList((AttributeList)endNode);
				break;
			case 6: // integer child: either IntegerExpression or NULL node
				startNode.setOffset((IntegerExpression)endNode);
				break;
	...
	// wzw
	private int handleArrowFunction( ArrowFuncExpression startNode, ASTNode endNode, int childnum)
	{
		int errno = 0;
		...
		switch (childnum)
		{
			...
			case 1:	// docComment child: StringExpression node or NULL node
				...
				break;
			case 2: // params child: ParameterList node
				startNode.setParameterList((ParameterList)endNode);
				break;
			case 3:	// stmt child: either CompoundStatement node or NULL node
				if (endNode instanceof NullNode)
					startNode.addChild(endNode);
				else
					startNode.setContent((ReturnStatement) endNode);
				break;
			case 4:	// returnType child: either Identifier node or NULL node
				if (endNode instanceof NullNode)
					startNode.addChild(endNode);
				else
					startNode.setReturnType((Identifier)endNode);
				break;
			case 5:	//	attributes child: either AttributeList or NullNode
				if (endNode instanceof NullNode)
					startNode.addChild(endNode);
				else
					startNode.setAttributeList((AttributeList)endNode);
	...
	private int handleNullableType( NullableType startNode, ASTNode endNode, int childnum)
	{
		int errno = 0;
	...
	private int handleClassName( ClassNameExpression startNode, ASTNode endNode, int childnum)
	{
		int errno = 0;
		...
		switch (childnum)
		{
			...
			case 1: // default child: Expression or null node
				if( endNode instanceof NullNode)
					...
					startNode.setDefault((Expression)endNode);
				break;
			case 2: // docComment child: StringExpression or null node
				if ( endNode instanceof NullNode)
					startNode.addChild(endNode);
				else
					startNode.addChild((StringExpression)endNode);
				break;
	...
	// wzw: version 70
	private int handlePropertyGroup( PropertyGroup startNode, ASTNode endNode, int childnum)
	{
		int errno = 0;
		...
		switch (childnum)
		{
			...
			case 1: // props child: PropertyElement node
				...
				break;
			case 2:	// attributes child: AttributeList node or null node
				if ( endNode instanceof NullNode)
					startNode.addChild(endNode);
				else
					startNode.setAttributeList((AttributeList)endNode);
	...
	private int handleCatch( CatchStatement startNode, ASTNode endNode, int childnum)
	{
		...
		switch (childnum)
		{
			case 0: // exception child: IdentifierList node
				...
				break;
			case 1: // var child: either Variable node or Null Node (PHP 8.0 Non-capturing Catches)
				if (endNode instanceof NullNode)
					startNode.addChild(endNode);
				else
					startNode.setVariable((Variable)endNode);
		...
		switch (childnum)
		{
			...
			case 2: // default child: Expression or null node
				if( endNode instanceof NullNode)
					...
					startNode.setDefault((Expression)endNode);
				break;
			case 3:	// attribute child: AttributeList or null node
				if ( endNode instanceof NullNode)
					startNode.addChild(endNode);
				else
					startNode.setAttributeList((AttributeList) endNode);
				break;
			case 4:	// docComment child: StringExpression node or null node
				if ( endNode instanceof NullNode)
					startNode.addChild(endNode);
				else
					startNode.setDocComment((StringExpression)endNode);
	 ...
	 */
	private int handleUnionType( UnionType startNode, ASTNode endNode, int childnum)
	{
		startNode.addUnionTypeElement((Identifier)endNode);

		return 0;
	}

	private int handleAttributeList( AttributeList startNode, ASTNode endNode, int childnum)
	{
		// AttributeList can have multiple children( AttributeGroup ) nodes
		startNode.addChild(endNode);

		return 0;
	}

	private int handleAttributeGroup( AttributeGroup startNode, ASTNode endNode, int childnum)
	{

		startNode.addAttribute((Attribute)endNode);

		return 0;
	}

	private int handleAttribute( Attribute startNode, ASTNode endNode, int childnum)
	{
		int errno = 0;

		switch (childnum)
		{
			case 0:	// name child (Identifier)
				startNode.setAttrName((Identifier)endNode);
				break;
			case 1: // argumentList or NullNode
				if ( endNode instanceof NullNode)
					startNode.addChild(endNode);
				else
					startNode.setArgumentList((ArgumentList)endNode);
	...
	private int handleMatch( MatchExpression startNode, ASTNode endNode, int childnum)
	{
		int errno = 0;

		switch (childnum)
		{
			case 0:	// condition child: Expression node
				startNode.setCondition((Expression)endNode);
				break;
			case 1:	// stmt child: Statement node
				startNode.setMatchARMList((MatchARMList)endNode);
				break;
	...
	private int handleMatchARMList( MatchARMList startNode, ASTNode endNode, int childnum)
	{
		startNode.addChild(endNode);
	...
	private int handleMatchARMExpression( MatchARMExpression startNode, ASTNode endNode, int childnum)
	{
		int errno = 0;
	...
	private int handleNullsafeProperty( NullsafePropertyExpression startNode, ASTNode endNode, int childnum)
	{
		int errno = 0;
		...
		{
			case 0:	// expr child: Expression node
				startNode.setLeftExpression((Expression)endNode);
				break;
			case 1:	// prop child: Expression node
				startNode.setProperty((Expression)endNode);
				break;
	...
	private int handleNullsafeMethodCall( NullsafeMethodCallExpression startNode, ASTNode endNode, int childnum)
	{
		int errno = 0;

		switch (childnum)
		{
			case 0:	// expr child: Expression child
				startNode.setLeftExpression((Expression)endNode);
				break;
			case 1:	// method child: Expression child
				startNode.setMethod((Expression)endNode);
				break;
